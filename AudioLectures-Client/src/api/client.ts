    ...
    export class ApiClient {
        ...
        constructor(baseUrl?: string, http?: { fetch(url: RequestInfo, init?: RequestInit): Promise<Response> }) {
            this.http = http ? http : window as any;
            // בשלב זה, ה-baseUrl ייקח את הכתובת מתוך משתנה הסביבה
            this.baseUrl = baseUrl ?? "https://your-production-api-url.com/swagger/v1/swagger.json"
 ?? "http://localhost:5000";  // במקרה חירום אם משתנה הסביבה לא מוגדר
        }
        /**
        * @param body (optional) 
        * @return OK
